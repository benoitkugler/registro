...
func main() {
	...
	files[0], err = os.Create("create_1_tables.sql")
	check(err)
	files[1], err = os.Create("create_2_json_funcs.sql")
	check(err)
	files[2], err = os.Create("create_3_constraints.sql")
	check(err)
	defer files[0].Close()
	defer files[1].Close()
	defer files[2].Close()
	...
	for _, file := range files {
		_, err = file.WriteString("-- generated by make_sql.go DO NOT EDIT.\n\n")
		check(err)
	}
