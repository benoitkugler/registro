<template>
    <v-card :title="`Dossier de ${props.dossier.Dossier.Responsable}`" :subtitle="props.dossier.Dossier.Participants?.map((p) =>
        Personnes.label(p.Personne)
    ).join(', ')
        " class="ml-2">
        <!-- actions -->
        <template #append>
            <v-btn icon="mdi-pencil" @click="showEditDialog = true" class="mx-1"></v-btn>

            <v-tooltip text="Ajouter un paiement" location="top">
                <template #activator="{ props: tooltipProps }">
                    <v-btn icon size="small" class="mx-1" v-bind="tooltipProps" @click="emit('createPaiement')">
            ...
            <v-tooltip text="Envoyer un message" location="top">
                <template #activator="{ props: tooltipProps }">
                    <v-btn icon size="small" class="mx-1" v-bind="tooltipProps" @click="showMessage = true">
            ...
            <v-menu>
                <template #activator="{ props: menuProps }">
                    <v-btn v-bind="menuProps" icon="mdi-dots-vertical" size="small" class="mx-1">
                    </v-btn>
                </template>
                <v-list>
                    <v-list-item prepend-icon="mdi-link" @click="showLinks = true"
                        title="Afficher les identifiants"></v-list-item>
                    <v-divider></v-divider>
                    <v-list-item prepend-icon="mdi-file-move" @click="
                        mergeParams = {
                            From: props.dossier.Dossier.Dossier.Id,
                            To: 0 as Int,
                            Notifie: true,
                        }
                        " title="Fusionner vers ..."></v-list-item>
                    <v-divider></v-divider>
                    <v-list-item prepend-icon="mdi-delete" @click="showDeleteDialog = true"
                        title="Supprimer"></v-list-item>
                </v-list>
            </v-menu>
        </template>
        ...
        <v-card-text>
            ...
            <v-row>
                <v-col class="ml-2 mb-1 text-center">
                    <v-menu>
                        <template #activator="{ props: menuProps }">
                            <v-chip v-bind="menuProps" prepend-icon="mdi-currency-eur"
            ...
            <div class="overflow-y-auto" style="max-height: 65vh">
                <v-timeline side="end" class="mt-4" density="compact">
                    <EventSwitch :event="event" v-for="(event, i) in events" :key="i"
        ...
        <v-dialog v-if="participantToCreate != null" :model-value="participantToCreate != null"
            @update:model-value="participantToCreate = null" max-width="600px">
            <v-card title="Ajouter un participant">
                <v-card-text>
                    <v-row>
                        <v-col cols="12">
                            <SelectCamp label="Camp" :camps="props.camps"
                                :model-value="zeroableToNullable(participantToCreate!.IdCamp)" @update:model-value="
                                    (v) => (participantToCreate!.IdCamp = nullableToZeroable(v))
                                "></SelectCamp>
                        </v-col>
                        <v-col cols="12">
                            <SelectPersonne label="Personne" initial-personne=""
                                :model-value="zeroableToNullable(participantToCreate!.IdPersonne)" @update:model-value="
                ...
                <v-card-actions>
                    <v-spacer></v-spacer>
                    <v-btn color="green" :disabled="participantToCreate.IdPersonne == 0 ||
        ...
        <v-dialog v-model="showEditDialog">
            <v-card title="Modifier le dossier">
                ...
                <v-card-text>
                    <v-row>
                        <v-col cols="3">
                            <DossierEditCard :responsable="props.dossier.Dossier.Responsable"
                                :dossier="props.dossier.Dossier.Dossier" @save="(v) => emit('updateDossier', v)">
                            </DossierEditCard>
                        </v-col>
                        <v-divider thickness="4" vertical></v-divider>
                        <v-col>
                            <div class="my-1"></div>
                            <DossierParticipantRow v-for="participant in props.dossier.Dossier.Participants"
                                :participant="participant" :aides="(props.dossier.Dossier.Aides || {})[
                                    participant.Participant.Id
                                ]
                                    " :structures="props.structures" :has-many-participants="(props.dossier.Dossier.Participants?.length || 0) > 1
                                        " @create-aide="(v) => emit('createAide', v)"
                                @delete-aide="(v) => emit('deleteAide', v)" @update-aide="(v) => emit('updateAide', v)"
                                @update="(p) => emit('updateParticipant', p)"
                                @delete="emit('deleteParticipant', participant.Participant.Id)"
                                @expand="emit('expandParticipant', participant.Participant)"></DossierParticipantRow>
                        </v-col>
                    </v-row>
                </v-card-text>
            </v-card>
        ...
        <!-- paiement editor -->
        <v-dialog v-if="paiementToUpdate != null" :model-value="paiementToUpdate != null"
            @update:model-value="paiementToUpdate = null" max-width="600px">
            <PaiementEditCard :paiement="paiementToUpdate" @update="
                (p) => {
                ...
                <v-card-text>
                    <v-row>
                        ...
                        <v-col class="text-truncate">
                            <a :href="props.dossier.EspacepersoURL" target="_blank">{{ props.dossier.EspacepersoURL }}
                            </a>
                        </v-col>
                        <v-col align-self="center" cols="auto">
                            <v-btn size="small" icon="mdi-content-copy" @click="copyEspacepersoURL"></v-btn>
                        </v-col>
                    </v-row>
                    <v-row>
                        <v-col cols="3">RIB ({{ asso }})</v-col>
                        <v-col cols="3">{{ props.dossier.BankName }}</v-col>
                        <v-col>{{ props.dossier.BankIBAN }}</v-col>
                        <v-col align-self="center" cols="auto">
                            <v-btn size="small" icon="mdi-content-copy" @click="copyBankIBAN"></v-btn>
                        </v-col>
                    </v-row>
                    <v-row>
                        ...
                        <v-col align-self="center" cols="auto">
                            <v-btn size="small" icon="mdi-content-copy" @click="copyVirementCode"></v-btn>
                        </v-col>
                    </v-row>
                </v-card-text>
            </v-card>
        ...
        <!-- merge dialog -->
        <v-dialog v-if="mergeParams != null" :model-value="mergeParams != null" @update:model-value="mergeParams = null"
            max-width="1000px">
            <v-card title="Fusionner le dossier vers"
                subtitle="Les participants, paiements et messages seront copiés vers le dossier cible.">
                <v-card-text>
                    <v-row>
                        <v-col cols="8">
                            <DossierList v-model:query="mergeQuery" :camps="props.camps"
                                @click="(v) => (mergeParams!.To = v.Id)"></DossierList>
                        </v-col>
                        <v-col cols="4">
                            <v-checkbox density="comfortable" label="Notifier par email"
                                hint="Le reponsable du dossier absorbé sera averti du changement d'espace personnel."
                                persistent-hint v-model="mergeParams.Notifie"></v-checkbox>
                        </v-col>
                    </v-row>
                </v-card-text>
                <v-card-actions>
                    <v-spacer></v-spacer>
                    <v-btn :disabled="mergeParams.To == 0 || mergeParams.To == mergeParams.From
                        " @click="
        ...
        <v-dialog v-model="showMessage" max-width="600px">
            <v-card title="Nouveau message"
                ...
                <v-card-text>
                    <v-row>
                        <v-col>
                            <v-textarea rows="5" variant="outlined" v-model="messageContenu"></v-textarea>
                        </v-col>
                    </v-row>
                </v-card-text>
                <v-card-actions>
                    <v-spacer></v-spacer>
                    <v-btn :disabled="!messageContenu.length" @click="
...
async function copyBankIBAN() {
    await copyToClipboard(props.dossier.BankIBAN);
